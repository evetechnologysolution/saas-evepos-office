...
TableViewForReservation.propTypes = {
  selectedDate: PropTypes.any,
  onClickEffect: PropTypes.func,
};

export default function TableViewForReservation(props) {
  ...
  const getColor = (id, status) => {
    ...
    if (status === "Close" && props.selectedDate === today) {
      return theme.palette.error.main;
    }
              ...
              {tableData.table.map((item, i) =>
                <Box
                  ...
                  sx={{
                    ...
                    ...(item.status === "Open" && props.selectedDate === today && {
                      "&:hover": {
                        // backgroundColor: item._id === tableId ? theme.palette.warning.dark : theme.palette.primary.dark
                        backgroundColor: theme.palette.primary.dark
                      },
                    ...
                    color: "white",
                    fontWeight: 600,
                    cursor: (item.status === "Open" && props.selectedDate === today) || (props.selectedDate > today) ? "pointer" : "default",
                  ...
                  onClick={() => {
                    if (props.selectedDate > today || (item.status === "Open" && props.selectedDate === today)) {
                      handleClicked(roomId, item._id, item.name);
                    }
                  }}
                >
                  {item.name}
