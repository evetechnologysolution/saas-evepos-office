...
// sections
import { CustomerTableToolbar, CustomerTableRow } from '../../sections/@dashboard/customer';
...
export default function CustomerList() {
  ...
  useEffect(() => {
    const getData = async () => {
      let url = `/customers?page=${controller.page + 1}&perPage=${controller.rowsPerPage}`;
  ...
  const handleDetailRow = (id) => {
    navigate(PATH_DASHBOARD.customer.view(paramCase(id)));
  };
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    if (selectedId) {
      await axios.delete(`/customers/${selectedId}`);
          ...
          <Card>
            ...
            >
              <div style={{ minWidth: "40%" }}>
                <CustomerTableToolbar filterName={search} onFilterName={handleSearch} onEnter={handleOnKeyPress} />
              </div>
              <Button
                variant="contained"
