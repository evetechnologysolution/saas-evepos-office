...
// sections
import MemberPointHistoryTableRow from "./MemberPointHistoryTableRow";
import MemberPointVoucherTableRow from "./MemberPointVoucherTableRow";
// routes
...
const TABLE_HEAD = [
  ...
  { id: "", label: "Order Date", align: "center" },
...
export default function MemberPointFormView({ currentData }) {
  const { user } = useAuth();
    ...
    <>
      <Card sx={{ p: 3 }}>
        <Stack direction="row" justifyContent="flex-end" sx={{ mb: 3 }} gap={1}>
          <Button variant="outlined" onClick={() => navigate(PATH_DASHBOARD.report.memberPoint)}>Back</Button>
        </Stack>
